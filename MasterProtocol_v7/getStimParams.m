...
% gets incorporated in loadStimWave
...
allpow = {5, 10, 15}; % power to be applied at each location, multiplied by num locations
alldur = {0.7}; % linear ramp gets added in loadStimWave, delay time-0.2=0.7
alldel = {0}; % delays
allstate = {'GoCue'};
...
%             dur{end+1} = alldur{k};
            ...
            amp{end+1} = allpow{i}*sum(allStimLoc{j});
            loc{end+1} = allLoc{j}; % for display purposes
...
%             end
            ...
            [xWavShort, yWavShort] = makeGalvoWav(travelTime, W, allStimLoc{j}, scanFreq, voltsPerMm, angleCorFactor);
