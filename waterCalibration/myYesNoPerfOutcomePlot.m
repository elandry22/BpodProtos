...
switch Action
    ...
    case 'update'
        ...
        if ~isempty(BpodSystem.Data.TrialTypes)
            ...
            updateTotalPerfPlot(TotalPerfHandle);
...
ticks = [1 1.5 2 2.5 3.25 4];
tickLabels = {'Reversal', 'Autowater', 'Antibias', 'Early', 'Left', 'Right'};

set(YesNoHandle,'TickDir', 'out','YLim', [0.75 4.25], 'YTick', ticks, 'YTickLabel', tickLabels, 'FontSize', 20);
%hold(AxesHandle, 'on');
xlim([0 81])
...
for i=1:length(ticks)
    tickLabels(i) = {[num2str(ticks(i)*100), '%']};
end

set(TotalPerfHandle,'XLim', [0 700], 'YLim', [0 1.05], 'YTick', ticks,'YTickLabel', tickLabels, 'FontSize', 20);



function initPerfText(TextLocation)

handles = {'All', 'Recent', 'R', 'L', 'NoResponse', 'Rewards'};
dispText = {'All:', 'Recent:', 'Right:', 'Left:', 'No Resp:', 'Rewards:'};
...
y = 2;%find(contains(BpodSystem.Data.dataToPlotStr, 'Autolearn'));
...
y = 1.5;%find(contains(BpodSystem.Data.dataToPlotStr, 'Autowater'));
...
if TrialNum>=Nrecent
    if sum(BpodSystem.Data.dataToPlot.NoResponse(TrialNum-(Nrecent-1):TrialNum)==0) > 0
        BpodSystem.Data.Perf.Recent(TrialNum) = sum(BpodSystem.Data.dataToPlot.Hit(TrialNum-(Nrecent-1):TrialNum)==1)/sum(BpodSystem.Data.dataToPlot.NoResponse(TrialNum-(Nrecent-1):TrialNum)==0);
    end
else
    if sum(BpodSystem.Data.dataToPlot.NoResponse(1:TrialNum)==0) > 0
        BpodSystem.Data.Perf.Recent(TrialNum) = sum(BpodSystem.Data.dataToPlot.Hit==1)/sum(BpodSystem.Data.dataToPlot.NoResponse(1:TrialNum)==0);
    end
end
set(BpodSystem.GUIHandles.Perf.Recent, 'String', [num2str(round(BpodSystem.Data.Perf.Recent(TrialNum)*100,0)),' %']);

if sum(BpodSystem.Data.dataToPlot.Right) > 0
    BpodSystem.Data.Perf.R(TrialNum) = sum(BpodSystem.Data.dataToPlot.Hit==1 & BpodSystem.Data.dataToPlot.Right==1)/sum(BpodSystem.Data.dataToPlot.Right==1 & BpodSystem.Data.dataToPlot.NoResponse==0);
    set(BpodSystem.GUIHandles.Perf.R, 'String', [num2str(round(BpodSystem.Data.Perf.R(TrialNum)*100,0)),' %']);
end

if sum(BpodSystem.Data.dataToPlot.Left) > 0
    BpodSystem.Data.Perf.L(TrialNum) = sum(BpodSystem.Data.dataToPlot.Hit==1 & BpodSystem.Data.dataToPlot.Left==1)/sum(BpodSystem.Data.dataToPlot.Left==1 & BpodSystem.Data.dataToPlot.NoResponse==0);
    set(BpodSystem.GUIHandles.Perf.L, 'String', [num2str(round(BpodSystem.Data.Perf.L(TrialNum)*100,0)),' %']);
end
...
set(BpodSystem.GUIHandles.Perf.Rewards, 'String', num2str(sum(BpodSystem.Data.dataToPlot.Hit==1)));


function updateTotalPerfPlot(TotalPerfHandle)
global BpodSystem

hold(TotalPerfHandle, 'off')
plot(TotalPerfHandle, 1:length(BpodSystem.Data.Perf.All), BpodSystem.Data.Perf.All, 'k', 'LineWidth', 2);
hold(TotalPerfHandle, 'on')
plot(TotalPerfHandle, 1:length(BpodSystem.Data.Perf.Recent), BpodSystem.Data.Perf.Recent, 'Color', [0.5 0.5 0.5], 'LineWidth', 1);
plot(TotalPerfHandle, 1:length(BpodSystem.Data.Perf.R), BpodSystem.Data.Perf.R, 'Color', [0 102/256 204/256], 'LineWidth', 1.5);
plot(TotalPerfHandle, 1:length(BpodSystem.Data.Perf.L), BpodSystem.Data.Perf.L, 'Color', [256/256 0/256 25.6/256], 'LineWidth', 1.5);
plot(TotalPerfHandle, 1:length(BpodSystem.Data.Perf.NoResponse), BpodSystem.Data.Perf.NoResponse, 'k:', 'LineWidth', 1.5);

set(TotalPerfHandle,'XLim', [0 700], 'YLim', [0 1.05], 'YTick', [0.0 0.25 0.5 0.75 1.0],'YTickLabel', {'0%', '25%', '50%', '75%', '100%'}, 'FontSize', 20);
