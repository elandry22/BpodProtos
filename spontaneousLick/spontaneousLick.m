...
setupGUIcallbacks('Antibias', @manualChangeAutolearn);
...
for TrialNum = 1:MAXTRIALS
    ...
    % select trials here
    TrialTypes(TrialNum) = 1; %Place left lick port in front of mouse
...
function manualChangeAntibias(hObject, ~, ~)
global S;
Antibias = get(hObject,'Value');
S.GUI.Autolearn =  Antibias;
mySet('Antibias',S.GUI.Antibias,'Value');
