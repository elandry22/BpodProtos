function [sma, params] = getSpontaneousLickStateMatrix(outputs, TrialNum)
global S 
...
if ~isempty(strfind(S.GUIMeta.Location.String{locationNum}, 'Box'))
    ...
    sma = AddState(sma, 'Name', 'GoCue', 'Timer', S.GUI.WaterValveTime,...
        ...
        'OutputActions', outputs.CueOutput{1:end});
    
elseif ~isempty(strfind(S.GUIMeta.Location.String{locationNum}, 'Rig'))
    
    sma = AddState(sma, 'Name', 'GoCue', 'Timer', S.GUI.WaterValveTime,...
        'StateChangeConditions', {'Tup', 'AnswerPeriod'},...
        'OutputActions', {outputs.CueOutput{1:end}, 'BNCState', 2});
    
end

sma = AddState(sma, 'Name', 'AnswerPeriod', 'Timer', S.GUI.AnswerPeriod,...
    'StateChangeConditions', {'Port1In', 'Licked','Tup','NoResponse'},...% send them to stopLickingReturn? do we want them to lick at least once? should that be a parameter? or lick as many times as they want?
    'OutputActions', []); 

if isempty(outputs.Response)
    ...
    sma = AddState(sma, 'Name', 'Licked', 'Timer', S.GUI.WaterValveTime,...
        'StateChangeConditions', {'Tup', 'StopLicking'},...
        'OutputActions', {outputs.Response{1:end}});
end
