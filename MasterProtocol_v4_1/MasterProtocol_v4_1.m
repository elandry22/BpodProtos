...
sendOutputWaveforms('COM5'); % send camera and stim waveforms to waveplayer
S.newWaveform = false;
...
%     end
    ...
    if ~bad
        if isfield(S, 'wavParams')
            if isfield(S.wavParams.stim, 'xpos')
                % variables get saved on first trial in
                % BpodSystem.Data.TrialParams{1}.wav.stim
                S.wavParams.stim.xpos = []; % keeps file from getting too large
                S.wavParams.stim.ypos = [];
                S.wavParams.stim.pow = [];
            end
        end
...
if ~params.giveStim
    ...
    disp(['     StimLoc:       ', params.stimLoc])
    disp(['     Duration:      ', num2str(params.stimDur), ' seconds'])
...
if ~isempty(strfind(S.GUIMeta.Location.String{locationNum}, 'Box3'))
    mySet('Stimulation',0,'Value');
    mySet('Stimulation','On','Enable');
