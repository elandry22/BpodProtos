...
switch S.GUIMeta.StimHemisphere.String{S.GUI.StimHemisphere}
    case 'Unilateral'
        allStimLoc = allStimLoc(1);% unilateral ALM
        allLoc = allLoc(1);
    case 'Bilateral'
...
%     ypos = [ypos, mmLoc(2, loc(i))*ones(1, tptsDwell),...
%         linspace(mmLoc(2, loc(i)), mmLoc(2, loc(i+1)), tptsTravel)];
    
%     xpos = [xpos, loc{i}(1)*ones(1, tptsDwell),...
%         linspace(loc{i}(1), loc{i+1}(1), tptsTravel)];
%     ypos = [ypos, loc{i}(2)*ones(1, tptsDwell),...
%         linspace(loc{i}(2), loc{i+1}(2), tptsTravel)];
end
