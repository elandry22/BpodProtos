...
if ~params.giveStim
    ...
    disp(['     StimState:     ', params.wav.stim.state{params.stimNum}])
    disp(['     Duration:      ', num2str(params.wav.stim.dur{params.stimNum}), ' seconds'])
    if strcmp(params.wav.stim.state{params.stimNum}, 'Delay')
        stimDelay = params.wav.stim.del{params.stimNum} - params.wav.stim.tStartDel - S.GUI.SamplePeriod;
        stimDelay = round(stimDelay,1);
    elseif strcmp(params.wav.stim.state{params.stimNum}, 'GoCue')
        stimDelay = params.wav.stim.del{params.stimNum} - params.wav.stim.tStartDel - S.GUI.SamplePeriod - S.GUI.DelayPeriod;
        stimDelay = round(stimDelay,1);
    end
    disp(['     Delay:         ', num2str(stimDelay), ' seconds'])
end
